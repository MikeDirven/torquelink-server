...
fun Route.getUserProfileRoute() {
    get<TorqueLinkUserRoutingV1.Profiles.ByUserId>(::getUserProfileRouteDoc) { resource ->
        TorqueLinkDatabase.executeAsync {
            val loadedUserProfile: UserProfiles.UserProfileDto = UserProfileDao.findById(
                resource.userId
            )?.toResponseWithoutSettings()
                ?: throw UserApiExceptions.UserProfileIdNotFoundException(resource.userId)

            call.respond(HttpStatusCode.OK, loadedUserProfile)
        }
    }
}